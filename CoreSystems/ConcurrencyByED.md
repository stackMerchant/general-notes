...
``` java
Lock lock = new ReentrantLock(); // ReentrantLock locks again
...
latch.countDown();
latch.await(); // waits until count = 0, can pass time too
```

- Pass latch in each thread’s runnable and do latch.countdown() in finally
- While in main method, do latch.await()
...
- Concurrent data structures // like ConcurrentHashMap, BlockingQueue
...
## For actors, or actor based concurrency:
